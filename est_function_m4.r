  ...
  if(id_m == 4){
    ...
    if (modeling_method == "RANDOM_FOREST") {
      ...
      rf_cube <- grid_latin_hypercube(
        finalize(mtry(), datB2_tbl[,2:21]),
	  trees(c(100,1900), trans=NULL),
        min_n(c(1,20), trans= NULL),
        #loss_reduction(),
    ...
    if (modeling_method == "XGBOOST") {
	  ...
	  trees(c(100,1800), trans=NULL),
        learn_rate(c(0.001, 0.5), trans= NULL),
        finalize(mtry(), datB2_tbl[,2:21]),
    ...
    if (modeling_method == "TREE") {
      ...
      rpart_cube <- grid_latin_hypercube(
        cost_complexity(),
        min_n(c(10,40), trans=NULL),
        tree_depth(c(4, 20), trans=NULL), 
        size = 30
      )
    ...
    if (modeling_method == "SVM") {
      ...
      svm_cube <- grid_latin_hypercube(
        ...
        rbf_sigma(c(10^-4,10^-1), trans= NULL),
        svm_margin(c(0, 0.2), trans=NULL), 
        size = 30
      )
      set.seed(123)
      svm_grid <- tune_grid(
